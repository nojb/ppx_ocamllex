...
let encode_lexdef def =
  ...
  chars_count := 0;
  let entry =
    let {name=entry_name; args; shortest; clauses=casedef} = def in
...
let followpos size entry =
  ...
  let entry, _, _ = entry in
  fill TransSet.empty entry.lex_regexp;
  v
...
let make_dfa lexdef =
  let (chars, entry) = encode_lexdef lexdef in
